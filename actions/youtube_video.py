...
def _handle_search(parameters: dict, player) -> str:
    """Open YouTube search results page WITHOUT playing any video."""
    query = parameters.get("query", "").strip()
    if not query:
        return "Please tell me what to search on YouTube, sir."

    if player:
        player.write_log(f"[YouTube] Search results: {query}")
    ...
    print(f"[YouTube] 🔎 Opening search results: {search_url}")
